...
import type { S2Concerns } from "@/lib/content/types";

export default function Section2Concerns({ data }: { data: S2Concerns }) {
  const concerns = data.concerns;
  return (
    <section id="s2" className="h-auto min-h-[600px] py-8 sm:h-auto sm:min-h-0 sm:py-16 lg:py-20 flex flex-col justify-center overflow-hidden" style={{ backgroundColor: "var(--brand-lighter)" }}>
        ...
        <div className="flex flex-col gap-6 sm:hidden mt-2 pt-2 pb-2 w-full max-w-[340px]">
          {concerns.map((c, i) => (
            <div key={i} data-reveal data-reveal-delay={i + 1} className="relative">
              <div className="relative ml-5 mr-6 border pt-5 pb-3 px-3" style={{ borderColor: "rgba(56,50,42,0.3)" }}>
                <span className="absolute z-10 flex items-center justify-center font-serif italic" style={{ top: "-1.4rem", left: "-1.2rem", fontSize: "3.0rem", color: "var(--brand-dark)", backgroundColor: "var(--brand-lighter)", padding: "0 6px", lineHeight: "1" }}>
                  {i + 1}
                </span>
                ...
                <p className="text-[10px] leading-[1.6] font-light text-justify relative z-0" style={{ color: "var(--brand-dark)", opacity: 0.85 }}>{c.desc}</p>
              </div>
            </div>
          ))}
        </div>
